...
echo ""
# ── Existing .env (preserve values) ────────────────────────────
env_get() { [[ -f .env ]] && grep -E "^${1}=" .env | tail -1 | cut -d= -f2- || true; }
EXISTING_CLIENT_ID="$(env_get ANTIGRAVITY_CLIENT_ID)"
EXISTING_CLIENT_SECRET="$(env_get ANTIGRAVITY_CLIENT_SECRET)"
EXISTING_REFRESH="$(env_get BRIDGE_REFRESH_TOKEN)"
EXISTING_API_KEY="$(env_get BRIDGE_API_KEY)"
EXISTING_ADMIN_KEY="$(env_get BRIDGE_ADMIN_KEY)"

echo -e "${BOLD}OAuth client${RESET}"
echo "────────────────────────────────────────────────────────────────"
echo ""
echo "The bridge impersonates the Antigravity CLI and needs its public OAuth client."
echo "These values are NOT in the repo (GitHub push-protection). Get them from the"
echo "opencode-antigravity-auth npm package:"
echo "  npm pack opencode-antigravity-auth && tar xzf opencode-antigravity-auth-*.tgz"
echo "  grep -rho 'CLIENT_[A-Z]* = \"[^\"]*' package/"
echo ""
read -rp "ANTIGRAVITY_CLIENT_ID [${EXISTING_CLIENT_ID:-required}]: " ANTIGRAVITY_CLIENT_ID
ANTIGRAVITY_CLIENT_ID="${ANTIGRAVITY_CLIENT_ID:-$EXISTING_CLIENT_ID}"
read -rp "ANTIGRAVITY_CLIENT_SECRET [${EXISTING_CLIENT_SECRET:+(keep existing)}${EXISTING_CLIENT_SECRET:-required}]: " ANTIGRAVITY_CLIENT_SECRET
ANTIGRAVITY_CLIENT_SECRET="${ANTIGRAVITY_CLIENT_SECRET:-$EXISTING_CLIENT_SECRET}"
if [[ -z "$ANTIGRAVITY_CLIENT_ID" || -z "$ANTIGRAVITY_CLIENT_SECRET" ]]; then
    error "ANTIGRAVITY_CLIENT_ID and ANTIGRAVITY_CLIENT_SECRET are required."
    exit 1
fi
# write them now so auth-login.py (standalone) can read them
touch .env && chmod 600 .env
grep -q '^ANTIGRAVITY_CLIENT_ID=' .env 2>/dev/null \
    && sed -i.bak "s|^ANTIGRAVITY_CLIENT_ID=.*|ANTIGRAVITY_CLIENT_ID=${ANTIGRAVITY_CLIENT_ID}|" .env \
    || echo "ANTIGRAVITY_CLIENT_ID=${ANTIGRAVITY_CLIENT_ID}" >> .env
grep -q '^ANTIGRAVITY_CLIENT_SECRET=' .env 2>/dev/null \
    && sed -i.bak "s|^ANTIGRAVITY_CLIENT_SECRET=.*|ANTIGRAVITY_CLIENT_SECRET=${ANTIGRAVITY_CLIENT_SECRET}|" .env \
    || echo "ANTIGRAVITY_CLIENT_SECRET=${ANTIGRAVITY_CLIENT_SECRET}" >> .env
rm -f .env.bak
success "OAuth client saved to .env"
...
echo "  [2] Log in interactively → opens Google OAuth (no server needed)"
if [[ -n "$EXISTING_REFRESH" ]]; then
echo "  [3] Keep the refresh_token already in .env"
echo "      NOTE: tokens created before Sep 2026 lack the 'aicode' scope -> re-login (option 2)"
fi
echo ""

BRIDGE_REFRESH_TOKEN=""
read -rp "Option [1/2$( [[ -n "$EXISTING_REFRESH" ]] && echo /3 )]: " AUTH_MODE
AUTH_MODE="${AUTH_MODE:-2}"

if [[ "$AUTH_MODE" == "3" && -n "$EXISTING_REFRESH" ]]; then
    BRIDGE_REFRESH_TOKEN="$EXISTING_REFRESH"
    success "Keeping existing refresh_token."
    ...
    info "Starting interactive OAuth login (standalone PKCE flow) ..."
    info "Open the link it prints, authorize, then paste the redirect URL."
    echo ""
    .venv/bin/python3 auth-login.py   # standalone: writes BRIDGE_REFRESH_TOKEN into .env
...
read -rp "Listen host [0.0.0.0] (use 127.0.0.1 to keep it local-only): " BIND_HOST
...
if [[ "$ACCT_MODE" == "2" ]]; then
    ...
    echo ""
    echo "Set an ADMIN key to protect the /admin/* endpoints (lists emails, creates accounts, starts logins)."
    GEN_ADMIN="$(python3 -c 'import secrets;print("sk-admin-"+secrets.token_urlsafe(24))')"
    read -rp "Admin key [${EXISTING_ADMIN_KEY:-generate: ${GEN_ADMIN}}] (type 'open' for no key): " ADMIN_KEY
    ADMIN_KEY="${ADMIN_KEY:-${EXISTING_ADMIN_KEY:-$GEN_ADMIN}}"
    if [[ "$ADMIN_KEY" == "open" ]]; then
        ADMIN_KEY=""
        warn "No admin key — /admin/* endpoints are OPEN. Only do this behind a firewall/tailnet."
    else
        success "Admin key set: ${ADMIN_KEY}"
    fi
...
# ── Write .env (all values; server.py loads it by itself) ───────
API_KEY="${API_KEY:-$EXISTING_API_KEY}"
cat > .env <<EOF
# Antigravity Bridge configuration (generated by install.sh $(date -u +%F))
HOST=${BIND_HOST}
PORT=${PORT}
BRIDGE_DEBUG=0

# OAuth client (public, but never committed)
ANTIGRAVITY_CLIENT_ID=${ANTIGRAVITY_CLIENT_ID}
ANTIGRAVITY_CLIENT_SECRET=${ANTIGRAVITY_CLIENT_SECRET}

# Google account (single-account mode)
BRIDGE_REFRESH_TOKEN=${BRIDGE_REFRESH_TOKEN}
EOF
[[ -n "$API_KEY" ]] && echo "BRIDGE_API_KEY=${API_KEY}" >> .env
ADMIN_KEY="${ADMIN_KEY:-$EXISTING_ADMIN_KEY}"
[[ -n "$ADMIN_KEY" ]] && echo "BRIDGE_ADMIN_KEY=${ADMIN_KEY}" >> .env
cat >> .env <<EOF

# Antigravity CLI fingerprint (override only if Google rotates them)
# ANTIGRAVITY_ASSIST_URL=https://daily-cloudcode-pa.googleapis.com/v1internal
# AGY_CLI_VERSION=1.1.28
# AGY_CLI_CL=978129418
# AGY_AUTH_METHOD=consumer
# AGY_CONSUMER_PROJECT=aicode-consumers
# BRIDGE_QUOTA_TTL=30
# BRIDGE_RESOLVE_CITATIONS=0
EOF
...
cd "${REPO_DIR}"
source .env
exec ${PYTHON_BIN_DIR}/python3 server.py --host ${BIND_HOST} --port ${PORT}
EOF
...
run_tests() {
    ...
    local cs; cs=$(curl -s -o /dev/null -w "%{http_code}" "${curl_auth[@]}" \
        ...
        -d '{"model":"gemini-3.8-flash-low","max_tokens":5,"messages":[{"role":"user","content":"reply with exactly: OK"}]}' \
    ...
    info "Testing /v1/quota ..."
    local qs; qs=$(curl -s -o /dev/null -w "%{http_code}" "${curl_auth[@]}" "$base_url/v1/quota" 2>/dev/null || true)
    [[ "$qs" == "200" ]] && success "/v1/quota OK (real Google quota)" || warn "/v1/quota returned ${qs}"

    info "Testing /docs ..."
    local ds; ds=$(curl -s -o /dev/null -w "%{http_code}" "$base_url/docs" 2>/dev/null || true)
    [[ "$ds" == "200" ]] && success "/docs OK -> ${base_url}/docs" || warn "/docs returned ${ds}"
}
...
if [[ "$INSTALL_DAEMON" =~ ^[Yy]$ ]]; then
    ...
    info "API docs (Swagger UI): http://127.0.0.1:${PORT}/docs"
fi
